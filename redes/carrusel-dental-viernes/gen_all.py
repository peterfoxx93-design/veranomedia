...
OUT = os.path.expanduser("~/veranomedia/redes/carrusel-dental-viernes")
W, H = 1080, 1350
...
# Portada
img = gradient_bg(top=(8,8,12), bot=(18,18,30)); d = ImageDraw.Draw(img)
f_badge = ImageFont.truetype(F_BOLD, 32)
f_h1 = ImageFont.truetype(F_BOLD, 86)
f_sub = ImageFont.truetype(F_REG, 40)
f_num = ImageFont.truetype(F_COND, 140)
badge = "CLÍNICAS DENTALES · VERANO MEDIA"
...
y += 40
y = draw_block(d, "CADA PACIENTE", f_h1, WHITE, W//2, y, 900, 10)
y = draw_block(d, "ES UNA OPINION", f_h1, WHITE, W//2, y, 900, 10)
y = draw_block(d, "EN GOOGLE.", f_h1, BLUE, W//2, y, 900, 10)
y += 30
draw_block(d, "Una experiencia mala se comparte. Una buena tambien.", f_sub, GREY, W//2, y, 860, 12)
...
y += 40
y = draw_block(d, "Una espera de 20 minutos", f_h, WHITE, W//2, y, 860, 12)
y = draw_block(d, "genera una mala reseña", f_h, WHITE, W//2, y, 860, 12)
y += 40
draw_block(d, "El paciente no olvida los tiempos de espera largos.", f_b, GREY, W//2, y, 840, 12)
draw_block(d, "Agenda inteligente = paciente contento.", f_b, GREY, W//2, y+70, 840, 12)
...
y = 200
y = number_block(d, "2", f_num_big, W//2, y, GREEN, alpha=40)
y += 40
y = draw_block(d, "Un tratamiento bien hecho", f_h, WHITE, W//2, y, 860, 12)
y = draw_block(d, "merece ser mostrado", f_h, WHITE, W//2, y, 860, 12)
y += 40
draw_block(d, "Las fotos de antes/despues son tu mejor publicidad.", f_b, GREY, W//2, y, 840, 12)
draw_block(d, "Nosotros te ayudamos a mostrarlas con estrategia.", f_b, GREY, W//2, y+70, 840, 12)
...
y += 40
y = draw_block(d, "Sin fidelizacion,", f_h, WHITE, W//2, y, 860, 12)
y = draw_block(d, "el paciente no vuelve", f_h, WHITE, W//2, y, 860, 12)
y += 40
draw_block(d, "Un paciente satisfecho puede traer 3 pacientes nuevos.", f_b, GREY, W//2, y, 840, 12)
draw_block(d, "Automatiza recordatorios y seguimientos post-consulta.", f_b, GREY, W//2, y+70, 840, 12)
...
f_b = ImageFont.truetype(F_REG, 40)
y = 160
y = draw_block(d, "Transforma cada consulta", f_h, WHITE, W//2, y, 920, 10)
y = draw_block(d, "en una historia de exito.", f_h, GREEN, W//2, y, 920, 10)
y += 70
draw_block(d, "Resenas + seguimiento + automatizacion para tu clinica.", f_b, GREY, W//2, y, 860, 10)
