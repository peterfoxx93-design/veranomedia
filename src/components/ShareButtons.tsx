const platforms = [
  ...
  { id: 'linkedin', label: 'LinkedIn', color: 'hover:bg-[#0A66C2] hover:text-white border-[#D1D1D6] hover:border-[#0A66C2]',
    ...
    shareUrl: (u: string, t: string, s?: string) => `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(u)}&title=${encodeURIComponent(t)}${s ? `&summary=${encodeURIComponent(s)}` : ''}` },
...
export default function ShareButtons({ slug, title, excerpt }: { slug: string; title: string; excerpt?: string }) {
  ...
  const handleShare = (platform: typeof platforms[0]) => {
    const shareUrl = platform.id === 'linkedin' && excerpt
      ? platform.shareUrl(url, fullTitle, excerpt)
      : platform.shareUrl(url, fullTitle)
