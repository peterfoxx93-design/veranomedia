...
const platforms = [
  ...
  },
  {
    ...
    shareUrl: null, // Instagram no tiene share URL web — se maneja con copia + deep link
...
export default function ShareButtons({ slug, title }: { slug: string; title: string }) {
  ...
  const handleShare = (platform: typeof platforms[0]) => {
    ...
    } else if (platform.customAction === 'copy-link') {
      navigator.clipboard.writeText(url).then(() => {
        setCopied(platform.id)
        setTimeout(() => setCopied(null), 2000)
      })
      // Intentar abrir Instagram app en móvil
      window.open('instagram://share?text=' + encodeURIComponent(fullTitle + ' ' + url), '_blank')
    } else {
      navigator.clipboard.writeText(url).then(() => {
        setCopied(platform.id)
        setTimeout(() => setCopied(null), 2000)
